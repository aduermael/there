...
/// Grass is generated per-frame by GPU compute shader (see grass_compute.wgsl).
pub fn scatter_objects(
    heightmap: &[f32],
) -> (Vec<RockInstance>, Vec<TreeInstance>) {
    ...
    log::info!(
        "Scatter: {} rocks, {} trees placed (grass generated by GPU compute)",
        rocks.len(),
        trees.len(),
    );
