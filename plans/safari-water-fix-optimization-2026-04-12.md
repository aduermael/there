...
- [ ] 4a: **Water shader** -- Reduce fragment noise cost. Currently 6 fbm3 for normals (2 layers x 3 finite-difference samples) + 2 fbm3 for foam = 8 fbm3 per pixel. Changes: (1) Drop the second ripple layer (ws2/wo2, lines 57-62) -- it contributes only 0.3 amplitude and is barely visible; fold a slight frequency variation into the primary layer instead. (2) Reduce foam from 2 fbm3 to 1 (single noise sample is sufficient for foam edge detection). Target: ~4 fbm3 per fragment (3 for normals + 1 for foam), down from 8.
