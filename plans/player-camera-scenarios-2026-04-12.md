...
- [ ] 2a: Add `steps` parsing to `ScenarioConfig`. Define step variants: `Input { forward, strafe, duration_secs }` and `Snapshot { output }`. All input fields default to 0.0.
...
- [ ] 3a: Add `move_direction(forward, strafe, yaw) -> (f32, f32)` and `move_yaw(forward, strafe, camera_yaw) -> f32` to `game-core/src/movement.rs`. Refactor `apply_movement()` to call `move_direction()` internally. Update `lib.rs:504-508` to call `move_yaw()`.
...
- [ ] 4a: Add `CAMERA_FOLLOW_SPEED` constant and `follow_behind(&mut self, move_yaw: f32, dt: f32)` method to `OrbitCamera` in `game-client/src/camera.rs`. Uses `game_core::movement::lerp_angle` to interpolate `self.yaw` toward `move_yaw + PI`.
- [ ] 4b: Integrate into frame loop in `lib.rs`: call `camera.follow_behind(local_move_yaw, dt)` after computing `local_move_yaw` and before `update_camera(dt)`, only when movement input is active.
- [ ] 4c: Add camera follow simulation to snapshot tool's step loop (Phase 2). During `Input` steps, after each movement tick, also interpolate the simulated orbit_yaw toward behind the movement direction at the same follow speed.
