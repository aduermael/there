...
- [ ] 1a: Remove `handle_root` redirect in `main.rs` — delete the `/` route so the fallback serves `web/index.html` directly. Visiting `/` now loads the game with no room code in the URL.
- [ ] 1b: Add `GET /api/rooms` endpoint in `main.rs` — reads `RoomManager` and returns JSON array `[{code, player_count}]` for all active rooms. Add a `list_rooms()` method to `RoomManager` that returns `Vec<(String, usize)>`.
- [ ] 1c: Add `GET /api/rooms/new` endpoint in `main.rs` — calls `generate_code()` and returns JSON `{code}`. Does not create the room (room is created on first WebSocket connect via `join_or_create`).
...
- [ ] 2a: Add a "Multiplayer" section to `game-menu.js` — positioned between the time presets and the Resume button. Contains: a room list area, a "Create Room" button, and a "Refresh" button. When the player is already in a room, show a "Leave Room" button instead.
